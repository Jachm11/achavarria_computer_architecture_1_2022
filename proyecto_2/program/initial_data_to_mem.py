...
def bytes_in_int_list_to_hex_list(L):
    res = []
    for i in L:
        if(i<= 15):
            res.append("0" + "{0:x}".format(i))
        else:
            res.append("{0:x}".format(i))
    return res 

def hex_in_str_list_to_word_in_str_list(L):
    res = []
    word = ''
    for i in range(0, len(L), 4):
        res.append(L[i+3] + L[i+2] + L[i+1] + L[i])
    return res
...
def main():
    ...
    txt_lines = bytes_in_int_list_to_hex_list(bytes)
    txt_lines = hex_in_str_list_to_word_in_str_list(txt_lines)
    write_txt(txt_lines)
