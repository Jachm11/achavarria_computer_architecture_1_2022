...
int has_station_in_use(conf_object_t * obj){
    connection_t *conn = conn_of_obj(obj);
    for(int i = 0; i < conn->total_reservation_stations; ++i){ 
...
void identify_instruction_and_operand(conf_object_t * obj, conf_object_t * cpu, logical_address_t address){ 
    ...
    int station_in_use = has_station_in_use(obj);
    SIM_LOG_INFO(1, obj, 0, "Has stations in use %x", station_in_use);
    if(station_in_use){
...
tomasulo_algorithm(conf_object_t *obj, conf_object_t *cpu, void *user_data){
    ...
    SIM_LOG_INFO(1, obj, 0, "Station in use %x adrress: %x", has_station_in_use(obj), (int)address);
    while(has_station_in_use(obj)){
        ++conn->cycles;
        SIM_LOG_INFO(1, obj, 0, "Current CPU Cycle: %i", conn->cycles);
        station_timers(obj);
        for(int i = 0; i < conn->total_reservation_stations; ++i){ 
...
gather_instruction_data(conf_object_t *obj, conf_object_t *cpu,
    ...
    SIM_LOG_INFO(3, obj, 0,"Add new instruction %s  stall: %x \n", disasm.string, (int)conn->stall);
