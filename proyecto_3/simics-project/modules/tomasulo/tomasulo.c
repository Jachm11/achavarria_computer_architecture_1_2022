...
void findDivOperand(char* instruction, char* operand1) {
    char* operandStart = strstr(instruction, " "); // Find the separator
    if (operandStart != NULL) {
        operandStart += strlen(" "); // Move the pointer past the separator
        char* commaPosition = strchr(operandStart, ','); // Find the comma separator
        if (commaPosition == NULL) {
            sscanf(operandStart, "%4s", operand1); // Extract the first operand
            return;
        }
    }
}
...
void parse_string(char* instruction, char* operand1, char* operand2) {
    char* operandStart = strstr(instruction, " "); // Find the separator
    if (operandStart != NULL) {
        operandStart += strlen(" "); // Move the pointer past the separator
        char* commaPosition = strchr(operandStart, ','); // Find the comma separator
        if (commaPosition != NULL) {
            size_t operandLength = commaPosition - operandStart; // Calculate the length of the first operand
            strncpy(operand1, operandStart, operandLength); // Copy the first operand without the comma
            operand1[operandLength] = '\0'; // Null-terminate the first operand
            operandStart = commaPosition + 1; // Move the pointer past the comma
            sscanf(operandStart, "%4s", operand2); // Extract the second operand
        } else {
            sscanf(operandStart, "%4s", operand1); // Extract the first operand
            operand2[0] = '\0'; // Set the second operand to an empty string
        }
    } else {
        operand1[0] = '\0'; // Set both operands to empty strings
        operand2[0] = '\0';
    }
}
...
bool check_finish_condition(conf_object_t * obj, instruction_entry* instruction){
    connection_t *conn = conn_of_obj(obj);

    char operand1[4];
    char operand2[4];
    parse_string(instruction->disassembled_text, operand1, operand2);
    SIM_LOG_INFO(2, obj, 0, "Checking if Operand 1 (dest) is writeable: %s", operand1);

    // Iterate through registers and check for operand match
    for (int i = 0; i < 16; i++) {
        if (strcmp(operand1, registers[i].name) == 0 ){
            bool is_ready = registers[i].read_locks == 0 || ((registers[i].read_locks - 1 == 0) && (strcmp(operand1, operand2) == 0 ));
            if (!is_ready){
                SIM_LOG_INFO(2, obj, 0, "Failed to finish, register %s still has %d read-locks", registers[i].name,registers[i].read_locks);
            }
            return is_ready;
        }
    }
...
bool try_read_lock(conf_object_t * obj, instruction_entry* instruction){
    connection_t *conn = conn_of_obj(obj);

    char operand1[4];
    char operand2[4];
    parse_string(instruction->disassembled_text, operand1, operand2);
    SIM_LOG_INFO(2, obj, 0, "Trying to read-lock operand 2 (src): %s", operand2);

    // Iterate through registers and check for operand match
    for (int i = 0; i < 16; i++) {
        if (strcmp(operand2, registers[i].name) == 0 ){
            if (!registers[i].write_locked){
                registers[i].read_locks += 1;
                SIM_LOG_INFO(2, obj, 0, "Success! Register %s # of read-locks incremented to %d", registers[i].name, registers[i].read_locks);
                return true;
            }
            else{
                SIM_LOG_INFO(2, obj, 0, "Failure, Register %s is write-locked", registers[i].name, registers[i].read_locks);
                return false;
            }
        }
    }
...
bool try_write_lock(conf_object_t * obj, instruction_entry* instruction){
    connection_t *conn = conn_of_obj(obj);

    char operand1[4];
    char operand2[4];
    parse_string(instruction->disassembled_text, operand1, operand2);
    SIM_LOG_INFO(2, obj, 0, "Trying to write-lock operand 1 (dest): %s", operand1);

    // Iterate through registers and check for operand match
    for (int i = 0; i < 16; i++) {
        if (strcmp(operand1, registers[i].name) == 0 ){
            if (registers[i].write_locked == 0){
                registers[i].write_locked = 1;
                SIM_LOG_INFO(2, obj, 0, "Success! Register %s is now write-locked", registers[i].name);
                return true;
            }
            else{
                SIM_LOG_INFO(2, obj, 0, "Failed to write-lock, register %s already write-locked", registers[i].name);
                return false;
            }
        }
    }
...
void free_locks(conf_object_t * obj, instruction_entry* instruction){
    connection_t *conn = conn_of_obj(obj);

    char operand1[4];
    char operand2[4];
    parse_string(instruction->disassembled_text, operand1, operand2);
    SIM_LOG_INFO(2, obj, 0, "Freeing write-lock on operand 1 (dest): %s", operand1);
    SIM_LOG_INFO(2, obj, 0, "Freeing read-lock on operand 2 (src): %s", operand2);

    // Iterate through registers and check for operand match
    for (int i = 0; i < 16; i++) {
        if (strcmp(operand1, registers[i].name) == 0 ){
            registers[i].write_locked = 0;
        }
        if (strcmp(operand2, registers[i].name) == 0 ){
            registers[i].read_locks--;
        }
    }
}
...
void identify_instruction_and_operand(conf_object_t * obj, conf_object_t * cpu, logical_address_t address){ 
    ...
    // Sort the prio_list based on station priorities
    for (int i = 0; i < conn->total_reservation_stations - 1; ++i) {
        for (int j = 0; j < conn->total_reservation_stations - i - 1; ++j) {
            int prio1 = conn->stations[conn->prio_list[j]]->priority;
            int prio2 = conn->stations[conn->prio_list[j + 1]]->priority;

            if (prio1 > prio2) {
                // Swap the positions in prio_list
                int temp = conn->prio_list[j];
                conn->prio_list[j] = conn->prio_list[j + 1];
                conn->prio_list[j + 1] = temp;
            }
        }
    }

    //Print prio list
    SIM_LOG_INFO(4, obj, 0, "Priority list");
    for (int i = 0; i < conn->total_reservation_stations; ++i) {
       SIM_LOG_INFO(4, obj, 0, "%d ", conn->prio_list[i]);
    }
   
    // Try to assign writelocks
    for(int i = 0; i < conn->total_reservation_stations; ++i){ 
        if (conn->stations[conn->prio_list[i]]->in_use && !conn->stations[conn->prio_list[i]]->write_lock){
            instruction_entry* trying_entry = get_instruction_details(obj, conn->stations[conn->prio_list[i]]->curr_inst->address);
            SIM_LOG_INFO(2, obj, 0, "Instruction %s, with index %d in priority list requesting for write-lock",trying_entry->disassembled_text,i);
            conn->stations[conn->prio_list[i]]->write_lock = try_write_lock(obj, trying_entry);
        }
    }

    SIM_LOG_INFO(1, obj, 0, "Finished giving write-locks");
    
    //Get most urgent finished functional unit
    int most_urgent = 0;
    int fu_ready = 0;
    for(int i = 0; i < conn->total_reservation_stations; ++i){ 
        if (conn->stations[i]->in_use && conn->stations[i]->current_timeout == 0){
            if (conn->stations[i]->priority < conn->stations[most_urgent]->priority){
                most_urgent = i; 
            } else if (!conn->stations[most_urgent]->in_use) {
                most_urgent = i;
    ...
    if (fu_ready){
        SIM_LOG_INFO(4, obj, 0, "Most urgent FU is %d",most_urgent);
    }

    // Try to finish a instruction
    if (fu_ready){
        conn->finished_address = conn->stations[most_urgent]->curr_inst->address; //Address of the finished instruction
        instruction_entry* finished_entry = get_instruction_details(obj, conn->finished_address);
        SIM_LOG_INFO(1, obj, 0, "We are gonna try to finish %s",finished_entry->disassembled_text);
        bool ready_to_finish = check_finish_condition(obj, finished_entry);
        
        SIM_LOG_INFO(1, obj, 0, "Write-lock is %d",conn->stations[most_urgent]->write_lock);

        if (ready_to_finish && conn->stations[most_urgent]->write_lock){
            ...
            conn->stations[most_urgent]->in_use = false; //since the instruction is finised, this engine is no longer busy
            conn->stall = false; //just in case we were stalling before
            conn->stations[most_urgent]->write_lock = false; //since the instruction is finised, this engine is no longer busy
            free_locks(obj, finished_entry);
            SIM_LOG_INFO(2, obj, 0, "I'm going to finish something here: %x and return to: %x\n",  conn->finished_address , conn->restore_rip_address);
        }
        else
        {
            SIM_LOG_INFO(2, obj, 0, "I was going to finish something here, but I cant\n");
        }
    }
    ...
    SIM_LOG_INFO(2, obj, 0, "Now we are gonna try to execute: %s", entry->disassembled_text);
    if(strncmp(entry->disassembled_text, "mov", 3) == 0) {
        for(int i = 0; i < conn->total_reservation_stations; ++i){ //Add instruction to station
            if(!conn->stations[i]->in_use && conn->stations[i]->type == TYPE_ALU){
                bool read_lock = try_read_lock(obj, entry);
                if (read_lock){
                    conn->stations[i]->curr_inst = entry;
                    conn->stations[i]->in_use = true;
                    conn->stations[i]->current_timeout = ALU_TIMEOUT; //we set the TIMEOUT to restart the instruction that just entered our station
                    conn->skip_instruction = true; //is already in a station, do not execute yet
                    bool write_lock = try_write_lock(obj, entry);
                    conn->stations[i]->write_lock = write_lock;
                    conn->stations[i]->priority = entry->address;
                    SIM_LOG_INFO(2, obj, 0, "Assigned to FU: %d",i);
                    return;
                }
                else {
                    SIM_LOG_INFO(1, obj, 0, "Could not execute");
        ...
        for(int i = 0; i < conn->total_reservation_stations; ++i){
            if(!conn->stations[i]->in_use && conn->stations[i]->type == TYPE_ALU){
                ...
                if (read_lock){
                    conn->stations[i]->curr_inst = entry;
                    conn->stations[i]->in_use = true;
                    conn->stations[i]->current_timeout = ALU_TIMEOUT;
                    conn->skip_instruction = true; //is already in a station, do not execute yet
                    bool write_lock = try_write_lock(obj, entry);
                    conn->stations[i]->write_lock = write_lock;
                    conn->stations[i]->priority = entry->address;
        ...
        for(int i = 0; i < conn->total_reservation_stations; ++i){ //Add instruction to station
            if(!conn->stations[i]->in_use && conn->stations[i]->type == TYPE_MUL){
                ...
                if (read_lock){
                    conn->stations[i]->curr_inst = entry;
                    conn->stations[i]->in_use = true;
                    conn->stations[i]->current_timeout = MUL_TIMEOUT; //we set the TIMEOUT to restart the instruction that just entered our station
                    conn->skip_instruction = true; //is already in a station, do not execute yet
                    bool write_lock = try_write_lock(obj, entry);
                    conn->stations[i]->write_lock = write_lock;
                    conn->stations[i]->priority = entry->address;
    ...
    if(!conn->finish){
        SIM_LOG_INFO(1, obj, 0, "No functional units available");
        SIM_LOG_INFO(1, obj, 0, "I need to stall");
        conn->stall = true;
        conn->stall_address = address; //we'll loop in the current RIP until some engine finished and removes the stall
        return;
    ...
    for(int i = 0; i < conn->total_reservation_stations; ++i){
        if(conn->stations[i]->in_use && conn->stations[i]->current_timeout > 0) {
            ...
            if(conn->stations[i]->type==0){
                SIM_LOG_INFO(1, obj, 0, "Station ALU decremented timeout to %x\n", conn->stations[i]->current_timeout);
...
void add_instruction_data(conf_object_t * obj, logical_address_t pa, char * disasm){
    ...
    if(strncmp(disasm, "div", 3) == 0){
        char operand[4];
        char ins[12];
        findDivOperand(strdup(disasm), operand);
        strcpy(ins, "div");
        strcat(ins, " ");
        strcat(ins, "eax");
        strcat(ins, ",");
        strcat(ins, operand);
        memcpy(disasm, ins, strlen(ins) + 1);
    }
...
void init_add_station(conf_object_t * obj, unit_functional_station * station){
    ...
    station->priority = MAX_INST;
    station->write_lock = 0;
}

void init_prio_list(conf_object_t * obj){
    connection_t *conn = conn_of_obj(obj);
    for (int i = 0; i < conn->total_reservation_stations; i++){
        conn->prio_list[i] = i;
    }
...
init_reg_file(void)
{
    registers[0].name = "eax";
    registers[1].name = "ebx";
    registers[2].name = "ecx";
    registers[3].name = "edx";
    registers[4].name = "esi";
    registers[5].name = "edi";
    registers[6].name = "ebp";
    registers[7].name = "esp";
    registers[8].name = "r8d";
    registers[9].name = "r9d";
    registers[10].name = "r10d"; //banned
    registers[11].name = "r11d"; //banned
    registers[12].name = "r12d"; //banned
    registers[13].name = "r13d"; //banned
    registers[14].name = "r14d"; //banned
    registers[15].name = "r15d"; //banned

    // Set .ready to true for all registers
    for (int i = 0; i < 16; i++) {
        registers[i].read_locks = 0;
        registers[i].write_locked = 0;
    }
}
